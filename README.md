`Dashboard` на основе gulp-scss темплейта и макета, предоставленных [dmgame](https://github.com/dmgame), в рамках учебного курса.
=====================
...
### Адаптивно! :+1: ###
### Производилась тестировка на хостинге (в т.ч. mobile) и валидация разметки. :+1: ###

 * @media расположены как следует.
 * Сброс применялся.
 * Cтили расписаны по компонентам.
 * Классы понятные.


Доработки требуют:
 * `id` / `for` (присвоить значения по факту, в т.ч. на элементы форм)
 * `notification line` (positive / negative) и условия, в которых они будут срабатывать.
 * Скриптовая связка `browserSync` + `scss` (подкорректировать).


###### Привожу список переменных из `_variables.scss`:
-------------
